...
if(isset($_POST['demandeAjout'])) {
    ...
    $sql1 = "INSERT INTO recette (titre,tpsPrep,tpsCui,diff,cout,calorie,video,".$sqlEtapesPartie1.") VALUES (:titre,:tpsPrep,:tpsCui,:diff,:cout,:calorie,:video,".$sqlEtapesPartie2.")";
    ...
    if(isset($_POST['validerVideo']) && $_POST['validerVideo']=="oui") {
        // recherche de l'id de la recette avec son nom qui vient d'etre ajoutee
        $sql4="SELECT id FROM recette WHERE titre=?";
        $q4=$pdo->prepare($sql4);
        $q4->execute(array($_POST['titre']));
        if($lineID=$q4->fetch()) {
    ...
    if(isset($_POST['ustensile'])) {
        ...
        $ustensileRecette=$_POST['ustensile'];
        $i=1;
        for($i=0;$i<sizeof($ustensileRecette);$i++) {
            $sql5 = 'INSERT INTO estDans (idRecette,idUstensile) VALUES ('.$idNouvelleRecette.','.$ustensileRecette[$i].')';
            $q5=$pdo->prepare($sql5);
            $q5->execute();
        }
